...
func TestGetDataprocRequestParams(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			want: DataprocRequestParams{
				...
				TemplateArgs: []string{"--template",
					...
					"--templateProperty",
					"jdbctospanner.sql=select gen_random_uuid() as synth_id, t.pk1, t.pk2, t.col3, t.col4 from test_schema.test_table as t",
					"--templateProperty",
					"jdbctospanner.output.instance=sp-test-instance",
					"--templateProperty",
					"jdbctospanner.output.database=sp-test-db",
		...
		{
			name: "valid postgres source profile and target profile with missing primary key in source",
			params: paramsStruct{
				sourceProfile: sourceProfilePostgres,
				targetProfile: targetProfile,
				srcSchema:     "test_schema",
