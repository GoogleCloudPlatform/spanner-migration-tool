...
// 	summarizer, err := NewMigrationSummarizer(ctx, &apiKey, projectID, location, mysqlSchema, spannerSchema, projectDir)
// 	if err != nil {
// 		logger.Log.Fatal("Error initializing summarizer: %v", zap.Error(err))
// 	}

// 	fileContent, _ := readFile(filePath)

// 	codeAssessment, result := summarizer.AnalyzeFile(ctx, filePath, "", fileContent)
// 	logger.Log.Debug("Code Assessment: first", zap.Any("codeAssessment", codeAssessment))

// 	publicMethod, err1 := json.MarshalIndent(result, "", "  ")
// 	if err1 != nil {
...
// 	fileContent1, _ := readFile(secondFilePath)
// 	codeAssessment, result = summarizer.AnalyzeFile(ctx, secondFilePath, string(publicMethod), fileContent1)
// 	logger.Log.Debug("Code Assessment: second", zap.Any("codeAssessment", codeAssessment))

// 	codeAssessment, err = summarizer.AnalyzeProject(ctx)
// 	logger.Log.Debug("Code Assessment: ", zap.Any("codeAssessment", codeAssessment))
// }
