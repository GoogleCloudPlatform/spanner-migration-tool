...
type MockPerformanceSchemaProvider struct {
	mock.Mock
}

func (m *MockPerformanceSchemaProvider) getPerformanceSchema(db *sql.DB, sourceProfile profiles.SourceProfile) (sourcesCommon.PerformanceSchema, error) {
	args := m.Called(db, sourceProfile)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).(sourcesCommon.PerformanceSchema), args.Error(1)
}

func TestDefaultPerformanceSchemaProvider_getPerformanceSchema(t *testing.T) {
	...
	provider := DefaultPerformanceSchemaProvider{}
	...
	psMySQL, err := provider.getPerformanceSchema(db, sourceProfileMySQL)
	...
	psUnsupported, err := provider.getPerformanceSchema(db, sourceProfileUnsupported)
	assert.Error(t, err)
...
func TestGetPerformanceSchemaCollector_Success(t *testing.T) {
	...
	mockPSProvider := new(MockPerformanceSchemaProvider)
	...
	mockPSProvider.On("getPerformanceSchema", dummyDb, sourceProfile).Return(mockPS, nil).Once()
	mockPS.On("GetAllQueryAssessments").Return(expectedQueries, nil).Once()

	collector, err := GetPerformanceSchemaCollector(sourceProfile, mockDbConnector, mockCfgProvider, mockPSProvider)
	...
	mockPSProvider.AssertExpectations(t)
...
func TestGetPerformanceSchemaCollector_ErrorFromGetConnectionConfig(t *testing.T) {
	...
	mockPSProvider.AssertNotCalled(t, "getPerformanceSchema", mock.Anything, mock.Anything)
}
...
func TestGetPerformanceSchemaCollector_ErrorFromPerformanceSchemaProvider(t *testing.T) {
	...
	mockCfgProvider.On("GetConnectionConfig", sourceProfile).Return("mock_conn_string", nil).Once()
	mockDbConnector.On("Connect", sourceProfile.Driver, "mock_conn_string").Return(dummyDb, nil).Once()
	mockPSProvider.On("getPerformanceSchema", dummyDb, sourceProfile).Return(nil, errors.New("mock provider error")).Once()
