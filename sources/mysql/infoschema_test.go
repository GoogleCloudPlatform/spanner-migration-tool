...
func TestProcessSchemaMYSQL(t *testing.T) {
	ms := []mockSpec{

		{
			...
			rows: [][]driver.Value{
				{"test", "ref", "id", "fk_test", "SET NULL", "CASCADE"},
			},
		}, {
		...
		},
		{
			...
			cols:  []string{"REFERENCED_TABLE_NAME", "COLUMN_NAME", "REFERENCED_COLUMN_NAME", "CONSTRAINT_NAME", "DELETE_RULE", "UPDATE_RULE"},
			rows: [][]driver.Value{
				{"product", "productid", "product_id", "fk_test2", "NO ACTION", "NO ACTION"},
				{"user", "userid", "user_id", "fk_test3", "RESTRICT", "SET NULL"},
			},
		}, {
			query: "SELECT (.+) FROM information_schema.COLUMNS (.+)",
			args:  []driver.Value{"test", "cart"},
		...
		},
		{
			...
			args:  []driver.Value{"test", "test"},
			cols:  []string{"REFERENCED_TABLE_NAME", "COLUMN_NAME", "REFERENCED_COLUMN_NAME", "CONSTRAINT_NAME", "DELETE_RULE", "UPDATE_RULE"},
			rows: [][]driver.Value{{"test_ref", "id", "ref_id", "fk_test4", "CASCADE", "RESTRICT"},
				{"test_ref", "txt", "ref_txt", "fk_test4", "CASCADE", "RESTRICT"}},
		}, {
			query: "SELECT (.+) FROM information_schema.COLUMNS (.+)",
			args:  []driver.Value{"test", "test"},
			cols:  []string{"column_name", "data_type", "column_type", "is_nullable", "column_default", "character_maximum_length", "numeric_precision", "numeric_scale", "extra"},
	...
	expectedSchema := map[string]schema.Table{
		"cart": schema.Table{Name: "cart", Schema: "test", ColIds: []string{"productid", "userid", "quantity"}, ColDefs: map[string]schema.Column{
			...
			ForeignKeys: []schema.ForeignKey{schema.ForeignKey{Name: "fk_test2", ColIds: []string{"productid"}, ReferTableId: "product", ReferColumnIds: []string{"product_id"}, OnDelete: "NO ACTION", OnUpdate: "NO ACTION", Id: ""}, schema.ForeignKey{Name: "fk_test3", ColIds: []string{"userid"}, ReferTableId: "user", ReferColumnIds: []string{"user_id"}, OnUpdate: "SET NULL", OnDelete: "RESTRICT", Id: ""}},
		...
		"test": schema.Table{Name: "test", Schema: "test", ColIds: []string{"id", "s", "txt", "b", "bs", "bl", "c", "c8", "d", "dec", "f8", "f4", "i8", "i4", "i2", "si", "ts", "tz", "vc", "vc6"}, ColDefs: map[string]schema.Column{
			...
			ForeignKeys: []schema.ForeignKey{schema.ForeignKey{Name: "fk_test4", ColIds: []string{"id", "txt"}, ReferTableId: "test_ref", ReferColumnIds: []string{"ref_id", "ref_txt"}, OnUpdate: "RESTRICT", OnDelete: "CASCADE", Id: ""}},
			Indexes:     []schema.Index(nil), Id: ""},
		...
		"user": schema.Table{Name: "user", Schema: "test", ColIds: []string{"user_id", "name", "ref"}, ColDefs: map[string]schema.Column{
			...
			ForeignKeys: []schema.ForeignKey{schema.ForeignKey{Name: "fk_test", ColIds: []string{"ref"}, ReferTableId: "test", ReferColumnIds: []string{"id"}, OnUpdate: "CASCADE", OnDelete: "SET NULL", Id: ""}},
			Indexes:     []schema.Index(nil), Id: ""}}
