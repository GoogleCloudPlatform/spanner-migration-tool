		...
		for _, i := range n.Cmds {
			...
			case *pg_query.Node_AlterTableCmd:
				...
				case a.Subtype == pg_query.AlterTableType_AT_AddIdentity && a.Name != "":
					// SQL-standard identity columns, introduced in PostgreSQL 10.
					// pg_dump emits these as a separate ALTER TABLE rather than an
					// inline CREATE TABLE constraint. ALWAYS and BY DEFAULT are treated
					// alike: both become GENERATED BY DEFAULT AS IDENTITY in Spanner.
...
func updateCols(ct pg_query.ConstrType, colNames []string, colDef map[string]schema.Column, colNameIdMap map[string]string) {
	...
	for _, cn := range colNames {
		...
		case pg_query.ConstrType_CONSTR_IDENTITY:
			...
			// Ignored.Default is intentionally not set: there is no user default to
			// drop, and the generation behaviour is reported via internal.IdentitySkipRange.
		}
		colDef[cid] = cd
	}
}
