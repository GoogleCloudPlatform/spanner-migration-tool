...
func TestProcessPgDump_Serial(t *testing.T) {
	serialColTests := []struct {
		...
		{
			name: "Identity column declared inline",
		...
		{
			name: "Identity column (ALWAYS) from pg_dump output",
			input: "CREATE TABLE public.identity_test (id bigint NOT NULL, col character varying(255));\n" +
		...
		{
			// BY DEFAULT maps to the same Spanner output as ALWAYS.
			name: "Identity column (BY DEFAULT) from pg_dump output",
			input: "CREATE TABLE public.identity_test (id integer NOT NULL, col character varying(255));\n" +
				"ALTER TABLE public.identity_test ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (\n" +
				"    SEQUENCE NAME public.identity_test_id_seq\n" +
				"    START WITH 1\n" +
				"    INCREMENT BY 1\n" +
				"    NO MINVALUE\n" +
		...
		{
			name: "Identity column in non-public schema from pg_dump output",
			input: "CREATE TABLE sales.identity_test (id bigint NOT NULL, col character varying(255));\n" +
				"ALTER TABLE sales.identity_test ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (\n" +
				"    SEQUENCE NAME sales.identity_test_id_seq\n" +
				"    START WITH 1\n" +
				"    INCREMENT BY 1\n" +
				");\n" +
				"ALTER TABLE ONLY sales.identity_test\n" +
				"    ADD CONSTRAINT identity_test_pkey PRIMARY KEY (id);",
			expectedSchema: map[string]ddl.CreateTable{
				"sales_identity_test": ddl.CreateTable{
					Name:   "sales_identity_test",
					ColIds: []string{"id", "col"},
					ColDefs: map[string]ddl.ColumnDef{
						"id":  ddl.ColumnDef{Name: "id", T: ddl.Type{Name: ddl.Int64}, NotNull: true, AutoGen: ddl.AutoGenCol{Name: constants.IDENTITY, GenerationType: constants.IDENTITY}},
						"col": ddl.ColumnDef{Name: "col", T: ddl.Type{Name: ddl.String, Len: 255}},
...
func TestProcessPgDump_IdentityDoesNotWarnAboutDefault(t *testing.T) {
	...
	for tableId, tbl := range conv.SpSchema {
		for _, colId := range tbl.ColIds {
			colName := tbl.ColDefs[colId].Name
			want, ok := wantIssues[colName]
			if !ok {
				continue
			}
			issues := conv.SchemaIssues[tableId].ColumnLevelIssues[colId]
			assert.Equal(t, want.identitySkipRange, slices.Contains(issues, internal.IdentitySkipRange),
