...
func (isi InfoSchemaImpl) GetConstraints(conv *internal.Conv, table common.SchemaAndName) (primaryKeys []string, checkConstrainst []schema.CheckConstraints, constraints map[string][]string, err error) {
	input := &dynamodb.DescribeTableInput{
		TableName: aws.String(table.Name),
	}
	result, err := isi.DynamoClient.DescribeTable(input)
	if err != nil {
		return primaryKeys, nil, constraints, fmt.Errorf("failed to make a DescribeTable API call for table %v: %v", table.Name, err)
	}
	...
	return primaryKeys, nil, constraints, nil
}
