...
func WriteSchemaFile(conv *internal.Conv, now time.Time, name string, out *os.File, driver string) {
	...
	spDDL := ddl.GetDDL(ddl.Config{Comments: true, ProtectIds: false, Tables: true, ForeignKeys: true, Indexes: true, SpDialect: conv.SpDialect, Source: driver}, conv.SpSchema, conv.SpSequences)
	if len(spDDL) == 0 {
	...
	spDDL = ddl.GetDDL(ddl.Config{Comments: false, ProtectIds: true, Tables: true, ForeignKeys: true, Indexes: true, SpDialect: conv.SpDialect, Source: driver}, conv.SpSchema, conv.SpSequences)
	if len(spDDL) == 0 {
		spDDL = []string{"\n-- Schema is empty -- no tables found\n"}
	}
	l = []string{
