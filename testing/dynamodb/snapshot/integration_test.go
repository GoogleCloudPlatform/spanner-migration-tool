...
func TestIntegration_DYNAMODB_Command(t *testing.T) {
	...
	args := fmt.Sprintf(`schema-and-data -source=%s -prefix=%s -target-profile="project=%s,instance=%s,dbName=%s"`, constants.DYNAMODB, filePrefix, projectID, instanceID, dbName)
