...
func checkCommonDataType(ctx context.Context, t *testing.T, client *spanner.Client) {
	...
	var timeVal string
	var moneyVal big.Rat
	iter := client.Single().Read(ctx, "AllTypes", spanner.Key{1}, []string{"Bit", "Date", "Float", "BigInt", "Numeric", "Text", "Time", "Money"})
	defer iter.Stop()
	for {
		row, err := iter.Next()
		...
		if err := row.Columns(&boolVal, &date, &floatVal, &intVal, &numericVal, &stringVal, &timeVal, &moneyVal); err != nil {
			t.Fatal(err)
		}
	}
