...
func TestIntegration_PGDUMP_SimpleUse(t *testing.T) {
	...
	err = cmd.CommandLine(conversion.PGDUMP, projectID, instanceID, dbName, false, false, false, 10, 0, "", &conversion.IOStreams{In: f, Out: os.Stdout}, filePrefix, now)
	if err != nil {
		t.Fatal(err)
	}
...
func TestIntegration_POSTGRES_SimpleUse(t *testing.T) {
	...
	err := cmd.CommandLine(conversion.POSTGRES, projectID, instanceID, dbName, false, false, false, 10, 0, "", &conversion.IOStreams{Out: os.Stdout}, filePrefix, now)
	if err != nil {
		t.Fatal(err)
	}
	// Drop the database later.
