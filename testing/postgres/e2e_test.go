...
func TestE2E_CheckIndexLimits(t *testing.T) {
	...
	testCases := []IndexLimitTestCase {
		...
		{
			name: "Spanner dialect with index name exactly 63 chars",

			dialect: constants.DIALECT_GOOGLESQL,
			ddls: []string{
				generateCreateTableDdl("t1", map[string]string{"p1": "bigint", "c1": "bigint"}, []string{"p1"}),
				generateCreateIndexDdl(strings.Repeat("i", 63), "t1", []string{"c1"}),
			},

			expectedNumberOfIndexesPerTable: map[string]int64{"t1": 1},
		},
		{
			name: "Postgres dialect with index name exactly 63 chars",
...
func runTableInterleaveLimitTestCase(t *testing.T, tmpdir string, tc TableInterleaveLimitTestCase, index int) {
	dbName := "postgres-interleave-limits"
	dbURI := fmt.Sprintf("projects/%s/instances/%s/databases/%s", projectID, instanceID, dbName)
	defer dropDatabase(t, dbURI)
	...
	args := fmt.Sprintf("schema -prefix %s -source=postgres -target-profile='instance=%s,dbName=%s,project=%s,dialect=%s' -session=%s < %s", filePrefix, instanceID, dbName, projectID, tc.dialect, sessionFileName, dumpFilePath)
	stdout, err := RunCommandReturningStdOut(args, projectID)
...
func createInitialSessionFile(filePrefix, dbName, dialect, dumpFilePath string) {
	args := fmt.Sprintf("schema -dry-run -prefix %s -source=postgres -target-profile='instance=%s,dbName=%s,project=%s,dialect=%s' < %s", filePrefix, instanceID, dbName, projectID, dialect, dumpFilePath)
