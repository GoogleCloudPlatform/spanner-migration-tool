...
func TestIntegration_MYSQLDUMP_SimpleUse(t *testing.T) {
	...
	err = cmd.CommandLine(conversion.MYSQLDUMP, projectID, instanceID, dbName, false, false, false, 10, 0, "", &conversion.IOStreams{In: f, Out: os.Stdout}, filePrefix, now)
	if err != nil {
		t.Fatal(err)
	}
...
func TestIntegration_MYSQL_SimpleUse(t *testing.T) {
	...
	err := cmd.CommandLine(conversion.MYSQL, projectID, instanceID, dbName, false, false, false, 10, 0, "", &conversion.IOStreams{Out: os.Stdout}, filePrefix, now)
	if err != nil {
		t.Fatal(err)
	}
	// Drop the database later.
