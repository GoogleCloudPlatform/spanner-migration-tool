	...
	for _, p := range []struct {
		...
		for _, colName := range colNames {
			...
			for _, i := range issues[colId] {
				...
				spColType := spSchema.ColDefs[colId].T.PrintColumnDefType(spSchema.ColDefs[colId].GeneratedColumn.IsVirtual())
				if conv.SpDialect == constants.DIALECT_POSTGRESQL {
					spColType = spSchema.ColDefs[colId].T.PGPrintColumnDefType(spSchema.ColDefs[colId].GeneratedColumn.IsVirtual())
				}
...
var IssueDB = map[internal.SchemaIssue]struct {
	...
	internal.Numeric:              {Brief: "Spanner does not support Postgres' arbitrary precision numeric type. This type mapping could lose precision and is not recommended for production use", Severity: warning, Category: "NUMERIC_USES"},
	internal.NumericThatFits:      {Brief: "Spanner does not support Postgres' arbitrary precision numeric type, but this type mapping preserves the numeric's specified precision", Severity: suggestion, Category: "NUMERIC_THAT_FITS"},
	internal.Decimal:              {Brief: "Spanner does not support Postgres' arbitrary precision decimal type. This type mapping could lose precision and is not recommended for production use", Severity: warning, Category: "DECIMAL_USES"},
	internal.DecimalThatFits:      {Brief: "Spanner does not support Postgres' arbitrary precision decimal type, but this type mapping preserves the decimal's specified precision", Severity: suggestion, Category: "DECIMAL_THAT_FITS"},
