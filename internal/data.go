...
// based on the specified Spanner and source DB schemas. Note that since entries
// in vals may be empty, we also return the list of columns (empty cols are dropped).
func ConvertData(conv *Conv, spTable, srcTable string, cols []string, vals []string) ([]string, []interface{}, error) {
	spannerSchema, ok1 := conv.spSchema[spTable]
	srcSchema, ok2 := conv.pgSchema[srcTable]
	if !ok1 || !ok2 {
	...
	for i, col := range cols {
		...
		srcCol, ok := srcSchema.cols[col]
		if !ok {
			return []string{}, []interface{}{}, fmt.Errorf("can't find col %s in srcSchema", col)
		}
		var x interface{}
		var err error
		if cd.IsArray {
			x, err = convArray(cd.T, srcCol, conv.location, vals[i])
		} else {
			x, err = convScalar(cd.T, srcCol.id, conv.location, vals[i])
		}
		if err != nil {
...
func convScalar(spannerType ddl.ScalarType, srcType string, location *time.Location, val string) (interface{}, error) {
	...
	// We do not expect src_dump to generate such output.
	...
	case ddl.String:
		return val, nil
	case ddl.Timestamp:
		return convTimestamp(srcType, location, val)
	default:
...
// pg_dump.
func convTimestamp(srcType string, location *time.Location, val string) (t time.Time, err error) {
	...
	if err != nil {
		return t, fmt.Errorf("can't convert to timestamp (posgres type: %s)", srcType)
	}
	return t, err
}

func convArray(spannerType ddl.ScalarType, srcType pgColDef, location *time.Location, v string) (interface{}, error) {
	...
	case ddl.Timestamp:
		var r []time.Time
		for _, s := range a {
			t, err := convTimestamp(srcType.id, location, s)
			if err != nil {
