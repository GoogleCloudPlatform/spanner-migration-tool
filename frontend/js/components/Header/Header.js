...
class Header extends HTMLElement {
  ...
  connectedCallback() {
    this.render();
    document.getElementById("schemaScreen").addEventListener('click',()=>{this.checkActiveSession()})
  }
  ...
  NavLinkTemplate(link) {
    return `
                  ...
                  <div class="header-topic">
                      <a name='${link.name}' href="${link.href}" id="${link.aTagId}" class='inactive'
  ...
  render() {
    ...
    this.innerHTML =
      logoTemplate + navLinks.links.map((link) => this.NavLinkTemplate(link)).join("");
  }
