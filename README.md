...
quality of the conversion. It also generates a schema DDL file and report file (and
a bad-data file if data was dropped and a schema conversion file if conversion 
mode is `schema-only`). See 
[Files Generated by HarbourBridge](#files-generated-by-harbourbridge).
Details of how source database's schema is mapped to Spanner can be found in the
...
- Schema DDL file (ending in `schema.ddl.txt`): contains the generated Spanner
  schema DDL, interspersed with comments that cross-reference to the relevant
  PostgreSQL/MySQL schema definitions.

- Schema conversion file (ending in `schema.conv.json`): contains all schema 
conversion state endcoded as JSON. This file is written for `-schema-only` mode.
