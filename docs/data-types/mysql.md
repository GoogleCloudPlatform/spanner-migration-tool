...
While moving from the schema preparation phase to the migration phase, any invalid or unsupported check constraints identified will be automatically dropped. The migration process will then be halted and will not proceed. This ensures the integrity and accuracy of the migration. Any identified issues will be clearly displayed in the Issues & Suggestions tab, allowing users to review all warnings in detail. This step is crucial to ensure that all constraints are compatible with the target system, thereby preventing potential issues during or after the migration. Once the necessary corrections are made, the user can re-initiate the migration process without any unsupported constraints.  
