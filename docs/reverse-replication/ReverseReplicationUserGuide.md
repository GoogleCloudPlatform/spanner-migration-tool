...
11. Reverse replication uses shard identifier column per table to route the Spanner records to a given source shard.The column identfied as the sharding column needs to be selected via HarbourBridge when perfroming migration. There are two ways to populate a sharding column when writing to Spanner:
    ...
    - The user populates it from the application code
