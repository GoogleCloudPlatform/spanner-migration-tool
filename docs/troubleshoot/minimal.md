...
gcloud dataflow flex-template run <jobName> \
 ...
 --parameters databaseId=<database id>,inputFilePattern=<gcs location of the datastream output>,dlqGcsPubSubSubscription=<pubsub subscription being used in a dlq gcs notification policy>,instanceId=<spanner-instance-id>,sessionFilePath=<GCS location of the session json>,streamName=<data stream name>,transformationContextFilePath=<path to transformation context json>
