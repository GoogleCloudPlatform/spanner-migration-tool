...
* **`defaultIdentitySkipRange`**: Optional flag. Specifies the default SKIP RANGE values to use for IDENTITY columns. Specified as "<min>-<max>", where both <min> and <max> are integers and <min> must be less than <max>. For example, "defaultIdentitySkipRange=10-50". For
