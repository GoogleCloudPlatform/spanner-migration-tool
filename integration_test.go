...
func TestIntegration_PGDUMP_SimpleUse(t *testing.T) {
	...
	err = toSpanner(PGDUMP, projectID, instanceID, dbName, "", int64(0), &ioStreams{in: f, out: os.Stdout}, filePrefix, now)
	if err != nil {
		t.Fatal(err)
	}
...
func TestIntegration_POSTGRES_SimpleUse(t *testing.T) {
	...
	err := toSpanner(POSTGRES, projectID, instanceID, dbName, "", int64(0), &ioStreams{out: os.Stdout}, filePrefix, now)
	if err != nil {
		t.Fatal(err)
	}
	// Drop the database later.
