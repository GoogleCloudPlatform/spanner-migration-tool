...
func TestPrintAutoGenCol(t *testing.T) {
	tests := []struct {
		...
		{AutoGenCol{Name: "testCol", GenerationType: constants.IDENTITY}, " GENERATED BY DEFAULT AS IDENTITY (BIT_REVERSED_POSITIVE)"},
		{AutoGenCol{Name: "testCol", GenerationType: constants.IDENTITY, IdentityOptions: IdentityOptions{SkipRangeMin: "10", SkipRangeMax: "100"}}, " GENERATED BY DEFAULT AS IDENTITY (BIT_REVERSED_POSITIVE SKIP RANGE 10, 100)"},
		{AutoGenCol{GenerationType: "", Name: ""}, ""},
	}
	for _, tc := range tests {
		assert.Equal(t, tc.expected, tc.agc.PrintAutoGenCol(Config{SpDialect: constants.DIALECT_GOOGLESQL}))
	}
}
...
func TestPGPrintAutoGenCol(t *testing.T) {
	...
	for _, tc := range tests {
		assert.Equal(t, tc.expected, tc.agc.PGPrintAutoGenCol(Config{SpDialect: constants.DIALECT_POSTGRESQL}))
	}
}
