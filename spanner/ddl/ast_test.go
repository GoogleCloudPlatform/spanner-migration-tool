...
func TestPrintAutoGenCol(t *testing.T) {
	tests := []struct {
		...
		// UUID generation depends on the column type: GENERATE_UUID() returns a
		// STRING, so a UUID-typed column needs NEW_UUID() instead.
		{AutoGenCol{Name: constants.UUID, GenerationType: "Pre-defined"}, Type{Name: String, Len: MaxLength}, " DEFAULT (GENERATE_UUID())"},
		{AutoGenCol{Name: constants.UUID, GenerationType: "Pre-defined"}, Type{Name: UUID}, " DEFAULT (NEW_UUID())"},
		{AutoGenCol{Name: "testSeq", GenerationType: constants.SEQUENCE}, Type{Name: Int64}, " DEFAULT (GET_NEXT_SEQUENCE_VALUE(SEQUENCE testSeq))"},
		{AutoGenCol{Name: "testCol", GenerationType: constants.IDENTITY}, Type{Name: Int64}, " GENERATED BY DEFAULT AS IDENTITY (BIT_REVERSED_POSITIVE)"},
		{AutoGenCol{Name: "testCol", GenerationType: constants.IDENTITY, IdentityOptions: IdentityOptions{SkipRangeMin: "10", SkipRangeMax: "100"}}, Type{Name: Int64}, " GENERATED BY DEFAULT AS IDENTITY (BIT_REVERSED_POSITIVE SKIP RANGE 10, 100)"},
		{AutoGenCol{GenerationType: "", Name: ""}, Type{Name: String, Len: MaxLength}, ""},
	}
	for _, tc := range tests {
		assert.Equal(t, tc.expected, tc.agc.PrintAutoGenCol(Config{SpDialect: constants.DIALECT_GOOGLESQL}, tc.ty))
	}
}
...
func TestPGPrintAutoGenCol(t *testing.T) {
	tests := []struct {
		...
		// spanner.generate_uuid() returns a string type, so a uuid
		// column needs gen_random_uuid().
		{AutoGenCol{Name: constants.UUID, GenerationType: "Pre-defined"}, Type{Name: String, Len: MaxLength}, " DEFAULT (spanner.generate_uuid())"},
		{AutoGenCol{Name: constants.UUID, GenerationType: "Pre-defined"}, Type{Name: PGUuid}, " DEFAULT (gen_random_uuid())"},
		{AutoGenCol{Name: "testSeq", GenerationType: constants.SEQUENCE}, Type{Name: Int64}, " DEFAULT NEXTVAL('testSeq')"},
		{AutoGenCol{Name: "testCol", GenerationType: constants.IDENTITY}, Type{Name: Int64}, " GENERATED BY DEFAULT AS IDENTITY (BIT_REVERSED_POSITIVE)"},
		{AutoGenCol{Name: "testCol", GenerationType: constants.IDENTITY, IdentityOptions: IdentityOptions{SkipRangeMin: "10", SkipRangeMax: "100"}}, Type{Name: Int64}, " GENERATED BY DEFAULT AS IDENTITY (BIT_REVERSED_POSITIVE SKIP RANGE 10 100)"},
		{AutoGenCol{GenerationType: "", Name: ""}, Type{Name: String, Len: MaxLength}, ""},
	}
	for _, tc := range tests {
		assert.Equal(t, tc.expected, tc.agc.PGPrintAutoGenCol(Config{SpDialect: constants.DIALECT_POSTGRESQL}, tc.ty))
	}
}

func TestPGPrintIdentityCol(t *testing.T) {
	tests := []struct {
		agc      AutoGenCol
		dialect  string
		expected string
	...
	for _, tc := range tests {
		assert.Equal(t, tc.expected, tc.agc.PrintIdentityCol(Config{SpDialect: tc.dialect}))
	}
}
