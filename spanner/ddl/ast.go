...
func (ty Type) PrintColumnDefType(isVirtual bool) string {
	str := ty.Name
	if ty.Name == String || ty.Name == Bytes {
		str += "("
		if ty.Len == MaxLength || isVirtual {
			// Virtual column require Sting size to be MAX
			str += "MAX"
		} else {
...
func (ty Type) PGPrintColumnDefType(isVirtual bool) string {
	str := GetPGType(ty)
...
func (gc GeneratedColumn) IsVirtual() bool {
	return gc.IsPresent && gc.Type == GeneratedColVirtual
}
