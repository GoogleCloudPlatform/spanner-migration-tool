...
export class PrepareMigrationComponent implements OnInit {
  ...
  initializeFromLocalStorage() {
    ...
    if (localStorage.getItem(MigrationDetails.HasDataMigrationCompleted) != null) {
      this.hasDataMigrationCompleted =
        (localStorage.getItem(MigrationDetails.HasDataMigrationCompleted) as string) === 'true'
    }
  ...
  openTargetDetailsForm() {
    ...
    dialogRef.afterClosed().subscribe(() => {
      ...
      if (
        this.targetDetails.TargetDB != '' ||
        (this.selectedMigrationType == MigrationTypes.lowDowntimeMigration &&
          this.targetDetails.StreamingConfig != '')
      ) {
          ...
          if (res.ErrorMessage == '') {
            ...
            // Checking for data migration in progree
              ...
              if (res.Progress == 100) {
                ...
                this.hasDataMigrationCompleted =
                  (localStorage.getItem(MigrationDetails.HasDataMigrationCompleted) as string) ===
                  'true'
              }
            }
  ...
  initializeLocalStorage() {
    ...
    localStorage.setItem(
      MigrationDetails.HasDataMigrationCompleted,
      this.hasDataMigrationCompleted.toString()
    )
    localStorage.setItem(
      MigrationDetails.DataMigrationProgress,
      this.dataMigrationProgress.toString()
